...
public class Main3 {
    ...
    public static void main(String[] args) throws Exception{
        ...
        printEquation(optimize(graph, 5, 1, 1000, 3),20);
    ...
    static double[] optimize(Map<Integer,Double[]> g, int n, int m, int t, double p){
        ...
        for(int a = 0; a < t; a++){ //for each iteration
            for(int k = 0; k < n+m; k++) { //for each coefficient
                Coefficient co = new Coefficient(g, period, n, m, c, num, maxy-miny, k);
                co.getErrors();
                co.minimize();
            }
        }
        ...
        perror /= Math.ceil((g.size()-num)/10.0);
        return c;
    }
...
class valueAtTime {
    ...
    double[] c;
    int n, m;
    valueAtTime(int n, int m, double period, double[] c, double x){
    ...
    double calculate(){
        double val = 0;
        for (int j = 0; j < n; j++) //for each cosine (c[j+n-1]*Math.pow(g.get(i)[0],j-1));
            val += (c[j]*Math.cos((((j+1)*2*Math.PI)/period)*x));
        for (int j = n; j < m+n; j++) val += c[j]*Math.pow(x,j-n);
        return val;
    }
}
class Coefficient {
    double[] x = new double[3], y = new double[3];
    Map<Integer,Double[]> g; double[] c;
    double period, range;
    int n, m, num, cn;
    Coefficient(Map<Integer,Double[]> g, double period, int n, int m, double[] c, int num, double range, int cn){
        this.g = g; this.c = c; this.num = num;
        this.period = period; this.range = range;
        this.m = m; this.n = n; this.cn = cn;
    }
    void getErrors(){
        for(int l = 0; l < 3; l++){
            x[l] = c[cn];
            for (int i = 0; i < num; i++) { //for every point
                valueAtTime v = new valueAtTime(n, m, period, c, g.get(i)[0]);
                y[l] += Math.pow(v.calculate() - g.get(i)[1], 2);
            }
            y[l] /= num;
            c[cn] = Math.random()*(range);
        }
    }
    void minimize(){
        ...
        c[cn] = .5*(((n1*n2)-(n3*n4))/((n1*n5)-(n6*n4)));
    }
}
